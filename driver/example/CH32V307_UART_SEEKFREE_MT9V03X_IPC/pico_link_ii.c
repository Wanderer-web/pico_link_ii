...
//------------用户替换函数需要包含的头文件-----------------------
#include "zf_common_headfile.h" // 逐飞ch32v307库版本3.1.1

//--------------------UART 宏定义------------------------------
#define CONNECT_CTS_PIN (1) // 是否接了流控引脚

// 读取CTS脚GPIO电平, 需要自己找函数定义, 该函数要有返回值, 0是低电平, 1是高电平
#define READ_CTS gpio_get_level((E8))

// UART发送多个字节，需要自己找函数定义, pBuff:发送内容首地址, len:发送长度
#define UART_SEND_BYTES(pBuff, len) uart_write_buffer((UART_7), (pBuff), (len))

//----------------------SPI 宏定义-----------------------------
// SPI发送多个字节，需要自己找函数定义, pBuff:发送内容首地址, len:发送长度
#define SPI_SEND_BYTES(pBuff, len) spi_write_8bit_array((SPI_3), (pBuff), (len))
