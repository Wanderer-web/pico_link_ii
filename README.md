...
|     空中速率      |       30Mbps       | **30Mbps (UDP) \| 20Mbps (TCP)** |
|     配置方式      |    串口+上位机     |           **WEB 配置**           |
|   UART 通信协议   |        定长        |            **不定长**            |
| 支持的 WiFi 频率  |       2.4Ghz       |              2.4Ghz              |
| 与上位机通信方式  |        UDP         |    **UDP(单向) \| TCP(双向)**    |
...
UDP 为单向发送到上位机，速度较快，适合只传图像的情形

TCP 可以和上位机双向通信，上位机向下位机发送的数据通过模块 UART 转发，模块处于 SPI 模式时 UART 波特率为115200，TCP 一次最多接收**128**字节，适合需要同时传图像和下位机调参的情形
...
![web_config_v1.1](image/web_config_v1.1.jpg)
